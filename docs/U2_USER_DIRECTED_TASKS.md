...
- `scripts/u2_user_runner.py`: 사용자 소유 프로세스로서 정책에 맞는 paused
  ...
  LaunchAgent 로그도 owner-only umask로 생성한다.
- `scripts/u2_user_runner.py configure-standing-review`: 기존 신뢰 SHA, 새로
  설치된 정확한 SHA, 서명 정책, 자동 발견 폴더를 다시 확인하고 외부 runner
  설정을 원자적으로 갱신한다. 같은 설정에 로컬 Command Center 서명 사건
  주소를 고정한다. Claude 호출이나 LaunchAgent 재시작은 하지 않는다.
- runner는 U2 queue를 11단계 진행 사건으로 바꾸어 controller key에서
